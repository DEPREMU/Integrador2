...
export const encryptHandler = async (
  socket: WebSocket,
  message: RequestEncrypt,
) => {
  const { dataToEncrypt } = message;
  ...
  try {
    response.dataEncrypted = encrypt(dataToEncrypt);
...
export const decryptHandler = async (
  socket: WebSocket,
  message: RequestDecrypt,
) => {
  const { dataToDecrypt } = message;
  ...
  try {
    response.dataDecrypted = decrypt(dataToDecrypt);
    socket.send(JSON.stringify(response));
  } catch (error) {
