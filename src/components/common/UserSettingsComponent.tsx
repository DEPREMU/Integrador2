...
const UserSettingsComponent: React.FC = () => {
    ...
    await updateUserData(updatedUserData, (success, error) => {
      if (error) {
        if (imageWasUploaded) {
          ...
          setImageWasUploaded(false); // Reset el flag
        } else {
          logError(error);
          setSnackbar({
            visible: true,
  ...
  /**
   * Renders the user profile image with upload functionality.
   * Handles multiple image URI formats (file://, content://, blob:, server paths).
   * Shows placeholder when no image is available.
   * Includes upload button for selecting new images.
   * @returns {JSX.Element} The image display and upload component
   */
  const renderImage = () => {
    let imageUri = null;
