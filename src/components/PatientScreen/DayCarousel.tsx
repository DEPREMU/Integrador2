...
import type { MedicationUser as MedicationUserBase } from "../../../server/src/types/Database";

// Extiende el tipo solo para el front
type MedicationUser = MedicationUserBase & { requiredDoses?: number };
...
const DayCarousel: React.FC<DayCarouselProps> = ({ medications, loading, onDeleteMedication }) => {
  ...
  const translateDosageType = (dosageType: string): string => {
    ...
    const rawDosageTypes = t("dosageTypes");
    if (Array.isArray(rawDosageTypes)) {
      dosageTypes = rawDosageTypes;
    } else if (typeof rawDosageTypes === 'string') {
      try {
        dosageTypes = JSON.parse(rawDosageTypes);
      } catch {
        dosageTypes = [];
      }
    }
    return index !== undefined && dosageTypes.length > index ? dosageTypes[index] || dosageType : dosageType;
  };
  ...
  const translateUrgency = (urgencyLevel: string): string => {
    ...
    let urgencyTranslations: Record<string, string> = {};
    const rawUrgency = t("urgency");
    if (typeof rawUrgency === 'object' && rawUrgency !== null) {
      urgencyTranslations = rawUrgency as Record<string, string>;
    } else if (typeof rawUrgency === 'string') {
      try {
        urgencyTranslations = JSON.parse(rawUrgency);
      } catch {
        urgencyTranslations = {};
      }
    }
    if (mappedKey && urgencyTranslations && urgencyTranslations[mappedKey]) {
      return urgencyTranslations[mappedKey];
    }
    return urgencyLevel;
  };
  ...
  const handleDeleteMedication = (medicationId: string, medicationName: string) => {
    ...
    if (onDeleteMedication) {
      ...
      setTimeout(() => {
        ...
        openModal(
          t("deleteMedication") || 'Delete Medication',
  ...
  useEffect(() => {
    ...
    // Prefer t('days') if available, else fallback to t(dayKey)
    const daysTranslations = t("days");
    const organizedDays: Day[] = dayNames.map((dayKey) => {
      let dayName = dayKey;
      if (daysTranslations && typeof daysTranslations === 'object' && typeof daysTranslations[dayKey] === 'string') {
        dayName = daysTranslations[dayKey];
      } else {
        const singleDay = t(dayKey);
        if (typeof singleDay === "string") dayName = singleDay;
      }
  ...
  const handleCardPress = (day: Day) => {
    ...
    openModal(
      `${t("medicationText") || 'Medications'} - ${day.name}`,
      hasmedications ? (
        <ScrollView 
          style={{ maxHeight: 400 }} 
            ...
            <View key={index} style={styles.modalMedicationCard}>
              <View style={{
                ...
                {onDeleteMedication && med._id && (
                  <Pressable
                    onPress={() => handleDeleteMedication(med._id ? med._id.toString() : "", med.name)}
                    style={({ pressed }) => [
                      {
                        padding: 8,
                        borderRadius: 20,
              ...
              <View style={styles.modalDetailRow}>
                ...
                <Text style={styles.modalDetailLabel}>{t("hour") || 'Time'}:</Text>
              ...
              <View style={styles.modalDetailRow}>
                ...
                <Text style={styles.modalDetailLabel}>{t("dosage") || 'Dosage'}:</Text>
                <Text style={styles.modalDetailValue}>
              ...
              <View style={styles.modalDetailRow}>
                ...
                <Text style={styles.modalDetailLabel}>{t("intervalHours") || 'Interval'}:</Text>
                <Text style={styles.modalDetailValue}>
                  {med.intervalHours} {med.intervalHours === 1 ? t("hour") : t("hours")}
                </Text>
              </View>

              <View style={styles.modalDetailRow}>
                <Ionicons name="cube-outline" size={16} color="#00a69d" />
                <Text style={styles.modalDetailLabel}>{t("stock") || 'Stock'}:</Text>
                <Text style={styles.modalDetailValue}>
                  {med.stock}
                </Text>
              </View>
              ...
              <View style={styles.modalDetailRow}>
                ...
                <Text style={styles.modalDetailLabel}>{t("requiredDoses") || 'Required Doses'}:</Text>
                <Text style={styles.modalDetailValue}>
                  {typeof med.requiredDoses === 'number' ? med.requiredDoses : 0}
                </Text>
              </View>
              ...
              <View style={styles.modalDetailRow}>
                <Ionicons 
                  ...
                  color="#00a69d"
                />
                <Text style={styles.modalDetailLabel}>{t("urgencyText") || 'Urgency'}:</Text>
                <Text style={[
                ...
                {day.medications.length > 0 ? (
                  day.medications.slice(0, 3).map((med, idx) => (
                    <View key={idx} style={styles.miniCard}>
                      <Text style={styles.medicationText}>{capitalize(med.name)}</Text>
                      <Text style={styles.medicationTime}>{formatTime(med.startHour)}</Text>
                      <Text style={styles.medicationDoses}>{t("requiredDoses")}: {typeof med.requiredDoses === 'number' ? med.requiredDoses : 0}</Text>
                    </View>
                  ))
                ) : (
                  <View style={styles.emptyMedication}>
...
// export default DayCarousel; (eliminada duplicada)
